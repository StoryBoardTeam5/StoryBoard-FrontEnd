...
const TypingTest = () => {
  ...
  const handleInput = (e: React.ChangeEvent<HTMLInputElement> ) => {
    ...
    if (lastLetter === ' ' || lastLetter === '.') {
      if (inputValue.includes(currentWord)) {   // current word has been typed
  ...
  return (
    <div id="body" className="flex h-screen content-center items-center justify-center bg-gray-900">
      <div id="Wrapper" className="w-1/2 rounded-lg bg-gray-700 p-8 text-white aspect-video" onClick={focusInput}>
        <input
          id="input"
          ...
          value={inputValue}
        />
        <div id="content-box" className="rounded-lg border-2 border-slate-600 p-4 aspect-video">
          <div id="typing-test" className="no-scrollbar max-h-60 overflow-y-auto">
            {!started || completed ? 'Press start to start' : prompt.split(' ').map((word, w_idx) => {
              let highlight = false
